...
resource "aws_security_group" "wp_rds_sg" {
  ...
  # SQL access from the public/private security groups
...
EOD
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --instance-ids ${aws_instance.wp_dev.id} --profile davidward && ansible-playbook -i aws_hosts wordpress.yml"
  }
}
...
resource "aws_launch_configuration" "wp_lc" {
  ...
  # This keeps up the instances before destroying them  when a chnage is made. 
  lifecycle {
